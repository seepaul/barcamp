...
exports.doRegister = function (req, res, next) {
  ...
  req.assert('tag1', 'Bitte überprüfe deine Eingabe').notEmpty().tag().isLength(2, 36);
  req.assert('tag2', 'Bitte überprüfe deine Eingabe').notEmpty().tag().isLength(2, 36);
  req.assert('tag3', 'Bitte überprüfe deine Eingabe').notEmpty().tag().isLength(2, 36);
  ...
  if (req.body.note) {
    req.assert('note', 'Bitte überprüfe deine Eingabe').optional();
  }
  ...
  if (req.body.session_title) {
    req.assert('session_title', 'Bitte überprüfe deine Eingabe').optional();
  }
  req.assert('session_desc').optional().isLength(0, 768);
  if (req.body.session_desc) {
    req.assert('session_desc', 'Bitte überprüfe deine Eingabe').optional();
  }
  req.assert('day_one').optional().equals('true');
  req.assert('day_two').optional().equals('true');
  req.assert('day_three').optional().equals('true');
  req.assert('twitter').optional().twitterUsername();
  req.assert('facebook').optional().facebookUsername();
  ...
  if (errors) {
    var err = new Error('Danke, aber wir konnten deine Eingabe nicht annehmen.');
    err.status = 403;
    err.message = util.inspect(errors);
    return next(err);
  }
