...
exports.nameTagList = function (req, res) {
  ...
  var csv = "id;firstName;lastName;tag1;tag2;tag3\n",
    row = 0;
...
exports.attendeeList = function (req, res) {
  ...
  var csv = "id;firstName;lastName;shirtSize;newcomer\n",
    row = 0;
  req.app.get('database').model.User.findAll(
    {
      where: {password: null},
    ...
    async.eachSeries(users, function (user, done) {
      row += 1;
      csv += row + ";" + user.firstName + ";" + user.lastName + ";" + user.shirtSize + ";" + user.newcomer;
      csv += "\n";
      done();
    }, function () {
      res.send(csv);
